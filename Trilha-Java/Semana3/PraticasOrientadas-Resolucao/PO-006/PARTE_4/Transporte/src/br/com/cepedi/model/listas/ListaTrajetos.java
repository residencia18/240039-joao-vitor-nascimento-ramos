...
public class ListaTrajetos extends ArrayList<Trajeto>  implements Serializable{

	private static final long serialVersionUID = 10L;

	public ListaTrajetos() {
		super();
	}

    public void carregarListaDoArquivo(String CAMINHO_ARQUIVO) {
    	ListaTrajetos listaLida = Persistencia.lerDoArquivo(CAMINHO_ARQUIVO);
        if (listaLida != null) {
    ...
    private void corrigeGeracaoID() {
    	for(Trajeto p : this) {
    		if(p.getId()>Trajeto.qntdIDsGerados) {
    			Trajeto.qntdIDsGerados=p.getId();
    		}
    	}    	
    }
